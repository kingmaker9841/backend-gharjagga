...
const storage = multer.diskStorage({
    destination: (req,file,done)=>{
        ...
        let folderName =  year + month + day + '-' + req.body.district_name + '-' + req.body.price + '-' + req.body.area + '-' + req.body.f_name + '-' + req.body.owner_phone_number + '-' + req.body.bedroom + '-' + req.body.bathroom + '-' + req.body.solar + '-' + req.body.title;
...
// Post a Apartment ad for session User
...
//To get all posted Apartment Ad of session user

router.get('/', ensureAuthenticatedUser, (req,res)=>{
    Apartment.find({user_id : req.session.user})
...
//To get specified Apartment with APARTMENT_ID

router.get('/:apartment_id', ensureAuthenticatedUser, (req,res)=>{
    Apartment.find({_id : req.params.apartment_id, user_id: req.session.user})
    .populate({path: 'owner_id', populate: [{path: 'prov_id'}, {path: 'district_id'}]})
    .populate({path : 'ghar_room_info', populate: [{path: 'floor'}, {path: 'bedroom'}, {path: 'bathroom'}, {path: 'kitchen'}, {path: 'living'}, {path: 'hall'}, {path: 'puja'}] })
    .populate({path: 'facilities_info', populate: [{path: 'balcony'}, {path: 'solar'}, {path: 'wifi'}, {path: 'tv'}, {path: 'furniture'}, {path: 'water_supply'}] })
    .populate('location')
...
//To delete specific APARTMENT with APARTMENT_ID

router.get('/delete/:apartment_id', ensureAuthenticatedUser, (req,res)=>{
    Apartment.findOneAndDelete({_id: req.params.apartment_id, user_id: req.session.user}, (err,result)=>{
        if (err){
...
//To update specific APARTMENT with APARTMENT_ID

router.post('/update/:apartment_id', ensureAuthenticatedUser, (req,res)=>{
    let {prov_number, district_name, f_name, l_name, owner_phone_number, floor, bedroom, bathroom, kitchen, living, hall, puja, balcony, solar, wifi, tv, furniture, water_supply, latitude, longitude} = req.body;
    let {price, title, near_by, description, address, price_type, purpose} = req.body;
    console.log(prov_number);
    ...
    async.waterfall([
        ...
        (owner_res, ghar_room_info_res, facilities_res, location_res ,done)=>{
            Apartment.findOneAndUpdate({_id: req.params.apartment_id , user_id: req.session.user} ,{
                owner_id: owner_res._id,
                ghar_room_info: ghar_room_info_res._id,
                facilities_info: facilities_res._id,
                price: price,
